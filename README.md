...
- 支持自定义周一至周日的整列背景高亮，默认高亮周六、周日；日期格展示系统日程颜色点。
...
当前 Release 使用 ad-hoc 临时签名，未进行 Apple Developer ID 签名或公证。如果 macOS 阻止启动，可右键点击应用选择“打开”，或在“系统设置 -> 隐私与安全性”中选择仍要打开。
...
推送 `v*` 标签会触发 GitHub Actions 自动构建 Release 版 App、应用 ad-hoc 临时签名、打包 DMG，并将 DMG 上传到对应的 GitHub Release。当前流程不依赖 Apple Developer 付费账号或额外的 GitHub Secrets。
